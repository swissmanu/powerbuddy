...
function register(task, simulate) {
  debug('register (simulate: ' + simulate || false + ')');
  ...
  if (task.action === Task.SHUTDOWN) {
    ...
    if (!simulate) {
      var at = childProcess.spawn('at', [time, date], {
      ...
      console.log('SIMULATE: Would schedule job with at for ' + date + ' ' + time);
      deferred.resolve();
    }
  } else {
    debug('task is no shutdown task');
    deferred.reject(new RangeError('Only shutdown tasks can be ' + 'scheduled using at.'));
  }
 ...
 *     (Promise)
 */
function unregister(ids, simulate) {
  debug('unregister (simulate: ' + simulate || false + ')');
  ...
  if (ids.length > 0) {
    ...
    if (!simulate) {
      ...
      console.log('SIMULATE: Would unschedule at job(s): ' + ids);
      deferred.resovle();
    }
  } else {
