...
function runCli(parameters) {
	...
	program
		.command('enable')
		.description('schedule the next upcoming PowerBuddy actions and ' +
			'prepare the system')
		.option('-s, --simulate', 'Simulation only. Do not write anything to ' +
			'the system')
		.action(executeEnable);

	program
		.command('disable')
		.description('remove possibly scheduled PowerBuddy actions from the ' +
			'system')
		.option('-s, --simulate', 'Simulation only. Do not write anything to ' +
			'the system')
