 ...
 *                    Optional. Default is the current date (as of `new Date()`).
 *
 * Returns:
 *     (Object)
 */
Schedule.prototype.getScheduleSheet = function getScheduleSheet(dayRange, today) {
	...
	var sheet = {}
		, firstDate = new Date(today.getFullYear(), today.getMonth(), today.getDate() - dayRange)
	...
	for(var i = 0, l = dayRange*2; i <= l; i++) {
		...
		sheet[date] = tasksGroupedByDay[dayOfWeek];
	}
 ...
 *
 * This is a relativliy expensive function since it contains two nested loops.
 * Use it with care :)
 *
 * Returns:
 *     (Object)
